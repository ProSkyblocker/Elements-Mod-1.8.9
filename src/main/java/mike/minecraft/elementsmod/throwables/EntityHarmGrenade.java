...
public class EntityHarmGrenade extends EntityThrowable {

	public EntityHarmGrenade(World worldIn) {
		super(worldIn);
	}

	public EntityHarmGrenade(World worldIn, EntityLivingBase throwerIn) {
		super(worldIn, throwerIn);
	}

	public EntityHarmGrenade(World worldIn, double x, double y, double z) {
		super(worldIn, x, y, z);
	}
				...
				for(int fx = 0; fx < 3; fx++){
					for(int fy = 0; fy < 3; fy++){
						for(int fz = 0; fz < 3; fz++){
							EntityPotion weak = new EntityPotion(worldObj, this.getThrower(), 16428);
